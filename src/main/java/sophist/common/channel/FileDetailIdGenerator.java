	...
	public Serializable generate(SharedSessionContractImplementor session, Object object) throws HibernateException {
	    ...
	    try {
	        ...
	        ResultSet rs=statement.executeQuery("select case when char_length(cast(file_detail_cd as varchar)) =1 then '00000000' ||  file_detail_cd "
	        		...
	        		+ " when char_length(cast(file_detail_cd as varchar)) =4 then '00000' || file_detail_cd"
	        		+ " when char_length(cast(file_detail_cd as varchar)) =5 then '0000' || file_detail_cd"
	        		+ " when char_length(cast(file_detail_cd as varchar)) =6 then '000' || file_detail_cd"
	        		+ " when char_length(cast(file_detail_cd as varchar)) =7 then '00' || file_detail_cd "
	        		+ " when char_length(cast(file_detail_cd as varchar)) =8 then '0' || file_detail_cd"
	        		+ " end "
